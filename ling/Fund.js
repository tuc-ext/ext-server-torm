...
DAD.api.getMyTokenBill = async function (option){
  ...
  if (txlist && txlist.status==='1') {
    ...
    let usdtTransactionList = await DAD.getOne({Fund: { uuidUser: option._passtokenSource.uuid}}) // 读取已有的该用户的交易列表
    ...
    hasNewTransaction ? DAD.setOne({Fund: {uuidUser: option._passtokenSource.uuid, usdtTransactionList}}) : ''
    return { 
      _state: 'SMOOTH', 
      usdtTransactionList,
    }
