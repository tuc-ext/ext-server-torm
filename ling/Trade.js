...
const EtherscanApi = require('etherscan-api').init(wo.config.ETHERSCAN_APIKEY, wo.config.ETH_NETTYPE, 5000)
...
DAD.api.getMyTokenBalance = async function ({ coinType, _passtokenSource } = {}) {
  if (_passtokenSource && coinType) {
    ...
    let tokenContract = wo.config.ETH_TOKEN_INFO[coinType].contract
...
DAD.api.refreshMyDeposit = async function ({ _passtokenSource, coinType } = {}) {
  ...
  if (wo.config.env === 'production') {
    switch (coinType) {
      case 'BTC':
      case 'USDT_ON_BTC':
        address = onlineUser.coinAddress.BTC.address
      case 'ETH':
      case 'USDT_ON_ETH':
        address = onlineUser.coinAddress.ETH.address.toLowerCase()
    }
    tokenContract = wo.config.ETH_TOKEN_INFO['USDT_ON_ETH'].contract
    txlistChain = await EtherscanApi.account.tokentx(address, tokenContract, startBlock, endBlock, pageNumber, pageSize, sort).catch(function (err) {
      console.log(err)
      return { _state: 'CHAIN_QUERY_EMPTY' }
    }) // 要做意外处理，因为etherscan-api的实现里，没钱的空账号竟然导致错误 “UnhandledPromiseRejectionWarning: No transactions found”
  } else {
      ...
      result: [
        {
          ...
          nonce: '9',
          blockHash: '0xecb630be18d6124da01bbdebfac7d230e07290a1482a885c214b29e644a523b7',
          from: '0xe72ba549597aec145b2ec62b99928bd8d1d16230',
          contractAddress: '0xb16815dbeceb459d9e33b8bba45ed717c479ea1c',
          to: '0x8900679eefef58d15fc849134e68577a17561155',
          value: '180800000000',
          tokenName: 'USDT',
          tokenSymbol: 'USDT',
          tokenDecimal: '6',
          ...
          gasUsed: '52394',
          cumulativeGasUsed: '6227960',
          input: 'deprecated',
          confirmations: '649552',
        },
    ...
    for (let txChain of txlistChain.result) {
      if (txChain.from === address) {
        ...
        if (!(await DAD.findOne({ uuidUser: _passtokenSource.uuid, txType: 'DEPOSIT_USDT', txHash: txHash }))) {
          ...
          txDB.exchangeRate = DAD.getExchangeRate({ coin: 'USDT_ON_ETH' }) // wo.config.coinSet.USDT_ON_ETH.exchange
