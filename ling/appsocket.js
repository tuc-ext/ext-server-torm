...
module.exports = {
  initSocket(webServer){
    ...
    my.appSocketServer.on('connection', (socket, req) => {
      ...
      socket.on('message', (data) => { // 在这里统一分发消息
        ...
        if (dataObj.skevent==='SOCKET_OWNER'){
          dataObj._passtokenSource = webToken.verifyToken(dataObj._passtoken, Config.tokenKey) // todo: 为防止前端欺骗，应当用和login里类似的方法来检查来检查
